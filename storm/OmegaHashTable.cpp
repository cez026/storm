...
/**********************************************************************************************************************
	Insert left reads in the hashTable
**********************************************************************************************************************/

bool OmegaHashTable::insertDataset_left(QueryDataset* d, UINT64 minOverlapLength)
{
	CLOCKSTART;
	MEMORYSTART;
	dataSet=d;
	...
	UINT64 size = getPrimeLargerThanNumber(d->getNumberOfUniqueReads() * 8 + 1);  // Size should be at least twice the number of entries in the hash table to reduce hash collision.
	setHashTableSize(size);
	for(UINT64 i = 1; i <= d->getNumberOfReads(); i++)		// For each read in the dataset
	{
		QueryRead * read = d->getReadFromID(i);
		if(read->getPairPosition()==1)
			hashRead_left(read); 								// Insert the read in the hash table.
		if(i%1000000 == 0)
			cout << setw(10) << i << " reads have been processed." << endl;	// Print some statistics.
	}
	cout << endl << "Total Hash collisions: " << numberOfHashCollision << endl;
...
bool OmegaHashTable::hashRead_left(QueryRead *read)
{
	string forwardRead = read->getSequence();
	string reverseRead = read->reverseComplement();
...
/**********************************************************************************************************************
	Insert right key of the reads in the hashTable for the right side alignment only
**********************************************************************************************************************/


bool OmegaHashTable::insertDataset_rightAlignOnly(QueryDataset* d, UINT64 minOverlapLength)
{
	CLOCKSTART;
	MEMORYSTART;
	dataSet=d;
	...
	for(UINT64 i = 1; i <= d->getNumberOfUniqueReads(); i++)		// For each read in the dataset
	{
		hashRead_left(d->getReadFromID(i)); 								// Insert the read in the hash table.
		if(i%1000000 == 0)
			cout << setw(10) << i << " reads inserted in the hash table. Hash collisions: " << setw(10) << numberOfHashCollision << endl;	// Print some statistics.
	}
	cout << endl << "Total Hash collisions: " << numberOfHashCollision << endl;
...
//		if(subStr == subString)
//				break;
		if(strcmp(subStr.c_str(), subString.c_str())==0)
				break;
